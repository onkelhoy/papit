...
import { getURL } from "components/http/url";
...
export function createInline(
    ...
    {
        [...sourceDocument.body.childNodes].forEach(node => document.body?.appendChild(node));
    }

    if (sourceDocument.body?.classList)
    {
        for (const className of sourceDocument.body?.classList)
        {
            document.body.classList.add(className);
        }
    }

    if (sourceDocument.body?.attributes)
    {
        for (const attr in sourceDocument.body?.attributes)
        {
            if (attr === "class") continue;
            if (sourceDocument.body.hasAttribute(attr)) continue;

            const value = sourceDocument.body.getAttribute(attr);

            if (value === true) 
            {
                sourceDocument.body.toggleAttribute(attr);
            }
            else 
            {
                sourceDocument.body.setAttribute(attr, value);
            }
        }
    }
    ...
    const _importmap: { imports: Record<string, string> } = JSON.parse(importmapScript.textContent || "{}");
