...
export class Cache {
    ...
    delete(url: ReturnType<typeof getURL>, noprint = false) {
        ...
        if (cached)
        {
            if (Arguments.verbose && !noprint) Terminal.write(Terminal.yellow(url.relative), "removed from cache:" + Terminal.blue(this.name))
    ...
    add(url: ReturnType<typeof getURL>, buffer: Buffer, mimeType?: string, mtime?: number | null) {
        ...
        if (Arguments.verbose) Terminal.write(Terminal.yellow(url.relative), "added to cache:" + Terminal.blue(this.name))
