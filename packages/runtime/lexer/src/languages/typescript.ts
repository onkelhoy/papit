...
export type TSToken =
    ...
    | { type: "commentStart"; value: string }
    | { type: "comment"; value: string }
    | { type: "commentEndDash"; value: string }
    | { type: "commentEnd"; value: string }
    | { type: "startTag"; name: string; attributes: Record<string, string | true>; selfClosing: boolean }
    | { type: "endTag"; name: string };

const tsRules: StateRules<CTX, TSToken> = {
    EOF: [
    ...
    markupDeclarationOpen: [
        { condition: "-", next: "commentStart" },  // first dash
        { condition: () => true, next: "doctype", action: (ctx, char) => ctx.append(char) },
    ],

    commentStart: [
        { condition: "-", next: "comment" },       // second dash, now inside comment body
        { condition: () => true, next: "data" },   // malformed, bail
    ],

    comment: [
        { condition: "-", next: "commentEndDash" },
        { condition: () => true, action: (ctx, char) => ctx.append(char) },
    ],

    commentEndDash: [
        { condition: "-", next: "commentEnd" },
        // not a --, go back and keep the - we consumed
        { condition: () => true, next: "comment", action: (ctx, char) => { ctx.append("-"); ctx.append(char); } },
    ],
    ...
    commentEnd: [
        {
            ...
            action: (ctx, char, buffer) => {
                ctx.emit({ type: "comment", value: buffer.trim() });
            }
        },
        // e.g. --->, keep going
        { condition: "-", action: (ctx, char) => ctx.append("-") },
        // not actually ending, e.g. --x
        { condition: () => true, next: "comment", action: (ctx, char) => { ctx.append("--"); ctx.append(char); } },
    ],
    //   markupDeclarationOpen: [
...
export function ts(value: string) {
    const lexer = new Lexer<CTX, TSToken>(
        tsRules,
        "data",
        {
            attrName: "",
