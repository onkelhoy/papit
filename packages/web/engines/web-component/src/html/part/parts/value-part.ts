import { UnsafeHTML } from "html/unsafe-html";
...
export class ValuePart implements Part {
    ...
    apply(newValue: any) {
        ...
        if (newValue instanceof UnsafeHTML && this.value instanceof UnsafeHTML && this.value.value === newValue.value) return;
        ...
        if (newValue instanceof UnsafeHTML)
        {
            this.clear(false);
            const temp = document.createElement('div');
            temp.innerHTML = newValue.value;
            const nodes: Node[] = [];
            while (temp.firstChild)
            {
                nodes.push(temp.firstChild);
                this.marker.parentNode?.insertBefore(temp.firstChild, this.marker);
            }
            this.unsafeNodes = nodes;
            return;
        }
    ...
    clear(value = true) {
        ...
        for (const node of this.unsafeNodes)
        {
